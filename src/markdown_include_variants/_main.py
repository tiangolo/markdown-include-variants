...
class Variant:
    ...
    version: Literal["py38", "py39", "py310", "py311"]
...
def calculate_variants(base_path: Path) -> dict[tuple[str, Union[bool, None]], Variant]:
    ...
    if an_p311_path.exists():
        ...
        if p311_path.exists():
            v = Variant(
                path=p311_path,
                title="Python 3.11+ - non-Annotated",
                is_annotated=False,
                version="py311",
            )
            variants[(v.version, v.is_annotated)] = v
    elif p311_path.exists():
        v = Variant(
            path=p311_path,
            title="Python 3.11+",
            is_annotated=None,
...
class IncludeVariantsPreprocessor(Preprocessor):
    def run(self, lines: list[str]) -> list[str]:
        ...
        for line in lines:
            ...
            file_title: Union[str, None] = None
            for config_match in re.finditer(re_config, configs_str):
                if config_match:
                    ...
                    if name == "ln":
                        ...
                        file_title = value_str.lstrip('"').rstrip('"')
            ...
            for an in [True, False, None]:
                for version in ["py311", "py310", "py39", "py38"]:
            ...
            if include_lines:
                ...
                if file_title:
                    first_line_full += f' title="{file_title}"'
                block_lines.extend(
                    [
            ...
            if len(sorted_variants):
                ...
                if file_title:
                    first_line_variant += f' title="{file_title}"'
                block_lines.extend(
                    [
